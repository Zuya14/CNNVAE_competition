...
def createLinearConvT(in_chs, out_chs, **kwargs):
    return nn.Sequential(
        nn.ConvTranspose1d(in_chs, out_chs, **kwargs),
        )
...
class EncoderInceptionStride(nn.Module):

    def __init__(self, in_channels, out_channels, stride, batchNorm=False):
        super().__init__()
        ...
        self.conv1 = createConv(in_channels, out_channels, kernel_size=1, stride=stride)

        self.conv3 = createConv(in_channels, out_channels, kernel_size=3, stride=stride, padding=1)

        self.conv3_1 = createConv(in_channels, in_channels, kernel_size=3, stride=stride, padding=1)
        self.conv3_2 = createConv(in_channels, out_channels, kernel_size=3, padding=1)

        # self.pool3 = nn.MaxPool1d(kernel_size=3, stride=stride, padding=1)
...
class DecoderInceptionStride(nn.Module):

    def __init__(self, in_channels, out_channels, stride, last=False, batchNorm=False):
        super().__init__()
        ...
        # self.conv3_1 = createConv(in_channels, out_channels, kernel_size=3, padding=1)
        # self.conv3_2 = createConv(out_channels, out_channels, kernel_size=3, stride=stride, padding=1)

        self.conv1 = createConv(in_channels, out_channels, kernel_size=1, stride=stride, output_padding= 1 if stride==2 else 2)

        self.conv3 = createConv(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, output_padding= 1 if stride==2 else 2)

        # self.conv3_1 = createConv(in_channels, out_channels, kernel_size=3, padding=1)
        # self.conv3_2 = createConv(out_channels, out_channels, kernel_size=3, stride=stride, padding=1, output_padding= 1 if stride==2 else 2)

        self.conv3_1 = createConv(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, output_padding= 1 if stride==2 else 2)
        self.conv3_2 = createConv(out_channels, out_channels, kernel_size=3, padding=1)

        # self.pool3 = nn.Upsample(scale_factor=stride, mode='nearest')
...
class Encoder(jit.ScriptModule):
    ...
    def __init__(self, first_channel, latent_size, repeat=0, batchNorm=False):
        ...
        if batchNorm:
            ...
            self.conv1 = EncoderInceptionStride(   1,    k, stride=3)
            self.conv2 = EncoderInceptionStride(   k,  2*k, stride=3)
            self.conv3 = EncoderInceptionStride( 2*k,  4*k, stride=3)
            self.conv4 = EncoderInceptionStride( 4*k,  8*k, stride=2)
            self.conv5 = EncoderInceptionStride( 8*k, 16*k, stride=2)
            self.conv6 = EncoderInceptionStride(16*k, 32*k, stride=2)
...
class Decoder(jit.ScriptModule):
    ...
    def __init__(self, last_channel, latent_size, repeat=0, batchNorm=False):
        ...
        if batchNorm:
            ...
            self.convT1 = DecoderInceptionStride(32*k, 16*k, stride=2)
            self.convT2 = DecoderInceptionStride(16*k,  8*k, stride=2)
            self.convT3 = DecoderInceptionStride( 8*k,  4*k, stride=2)
            self.convT4 = DecoderInceptionStride( 4*k,  2*k, stride=3)
            self.convT5 = DecoderInceptionStride( 2*k,    k, stride=3)
            self.convT6 = nn.Sequential(*[DecoderInceptionStride(   k,    1, stride=3, last=True), nn.Sigmoid()])
...
# class InceptionAllVAE(nn.Module):
class InceptionAllVAE(jit.ScriptModule):
...
if __name__ == '__main__':
    ...
    # model = EncoderInceptionStride(in_channels=1, out_channels=8, stride=3)
    ...
    # model = DecoderInceptionStride(in_channels=1, out_channels=8, stride=3)

    out = model(x)

    print(out.size())

    encoder = Encoder(first_channel=8, latent_size=18, repeat=0, batchNorm=False)

    mu, sigma = encoder(x)

    print(mu.size())

    decoder = Decoder(last_channel=8, latent_size=18, repeat=0, batchNorm=False)
    ...
    # encoderStride = EncoderInceptionStride(in_channels=1, stride=2)

    # out = encoderStride(x)

    # print(out.size())

    # encoderStride = EncoderInceptionStride(in_channels=1, stride=3)
    ...
    # decoderStride = DecoderInceptionStride(in_channels=1, stride=2)

    # out = decoderStride(x)

    # print(out.size())

    # decoderStride = DecoderInceptionStride(in_channels=1, stride=3)
